...
export const KNOWLEDGE_TEAMS_ER: ERDiagram = {
  ...
  entities: [
    ...
    },
    {
      ...
      attributes: [
        { name: "assistant", type: AttributeType.OBJECT, required: true, description: "SDKAssistantMessage — wraps BetaMessage with uuid, session_id, parent_tool_use_id" },
        { name: "user", type: AttributeType.OBJECT, required: true, description: "SDKUserMessage — wraps MessageParam with optional tool_use_result" },
        { name: "result.success", type: AttributeType.OBJECT, required: true, description: "num_turns, total_cost_usd, usage, modelUsage, duration_ms, structured_output?" },
        { name: "result.error", type: AttributeType.OBJECT, required: true, description: "subtype: error_max_turns|error_during_execution|error_max_budget_usd, errors[]" },
        { name: "system.init", type: AttributeType.OBJECT, required: true, description: "SDKSystemMessage — tools, agents, mcp_servers, model, permissionMode, claude_code_version" },
        { name: "stream_event", type: AttributeType.OBJECT, required: true, description: "SDKPartialAssistantMessage — real-time streaming content blocks" },
        { name: "compact_boundary", type: AttributeType.OBJECT, required: false, description: "SDKCompactBoundaryMessage — context compression trigger with pre_tokens count" },
      ],
    },
    {
      ...
      attributes: [
        { name: "sessionId", type: AttributeType.STRING, required: true, description: "UUID identifying this session" },
        { name: "summary", type: AttributeType.STRING, required: false },
        { name: "lastModified", type: AttributeType.TIMESTAMP, required: true },
        { name: "fileSize", type: AttributeType.INTEGER, required: false },
        { name: "customTitle", type: AttributeType.STRING, required: false },
        { name: "firstPrompt", type: AttributeType.STRING, required: false },
        { name: "gitBranch", type: AttributeType.STRING, required: false },
        { name: "cwd", type: AttributeType.STRING, required: false },
      ],
      functions: [
        { name: "listSessions", params: "(cwd?)", returnType: "Promise<SDKSessionInfo[]>", description: "List all persisted sessions (optionally filtered by cwd)" },
        { name: "getSessionMessages", params: "(sessionId)", returnType: "Promise<SessionMessage[]>", description: "Read full conversation history from .jsonl file" },
      ],
    },
    {
      ...
      attributes: [
        { name: "resume", type: AttributeType.STRING, required: false, description: "Session ID to resume" },
        { name: "continue", type: AttributeType.BOOLEAN, required: false, description: "Auto-find most recent session in cwd" },
        { name: "forkSession", type: AttributeType.BOOLEAN, required: false, description: "Create new session from copy of resumed session history" },
        { name: "persistSession", type: AttributeType.BOOLEAN, required: false, description: "false = memory-only (TS only), default true" },
        { name: "sessionId", type: AttributeType.STRING, required: false, description: "Explicit session ID to use" },
      ],
    },
    {
      ...
      attributes: [
        { name: "PreToolUse", type: AttributeType.STRING, required: false, description: "Before tool execution — can modify input or deny" },
        { name: "PostToolUse", type: AttributeType.STRING, required: false, description: "After tool execution — can modify output" },
        { name: "PostToolUseFailure", type: AttributeType.STRING, required: false, description: "After tool failure — error, is_interrupt?" },
        { name: "Notification", type: AttributeType.STRING, required: false, description: "Agent notifications — message, title?, notification_type" },
        { name: "UserPromptSubmit", type: AttributeType.STRING, required: false, description: "Before prompt processing — can modify prompt" },
        { name: "SessionStart", type: AttributeType.STRING, required: false, description: "TS only — source: startup|resume|clear|compact" },
        { name: "SessionEnd", type: AttributeType.STRING, required: false, description: "TS only — reason: ExitReason" },
        { name: "Stop", type: AttributeType.STRING, required: false, description: "Agent stop event — last_assistant_message?" },
        { name: "SubagentStart", type: AttributeType.STRING, required: false, description: "Sub-agent spawned — agent_id, agent_type" },
        { name: "SubagentStop", type: AttributeType.STRING, required: false, description: "Sub-agent completed — agent_transcript_path" },
        { name: "PermissionRequest", type: AttributeType.STRING, required: false, description: "Permission prompt — tool_name, tool_input, suggestions?" },
      ],
    },
    {
      ...
      attributes: [
        { name: "permissionMode", type: AttributeType.ENUM, required: true, enumValues: ["default", "acceptEdits", "bypassPermissions", "plan", "dontAsk"], description: "Global permission policy" },
        { name: "canUseTool", type: AttributeType.FUNCTION, required: false, description: "Custom callback: (toolName, input, opts) => Promise<PermissionResult>" },
        { name: "PermissionResult.allow", type: AttributeType.OBJECT, required: false, description: "{behavior: 'allow', updatedInput?, updatedPermissions?}" },
        { name: "PermissionResult.deny", type: AttributeType.OBJECT, required: false, description: "{behavior: 'deny', message, interrupt?}" },
      ],
    },
  ...
  relationships: [
    ...
    // ── Agent SDK Session & Hook Relationships ──────────────────────────

    {
      id: "rel.query__session",
      from: "agent_sdk.query",
      to: "agent_sdk.Session",
      kind: RelationshipKind.PRODUCES,
      cardinality: Cardinality.ONE_TO_ONE,
      label: "query() creates or resumes a Session (persisted .jsonl)",
    },
    {
      id: "rel.session_options__session",
      from: "agent_sdk.SessionOptions",
      to: "agent_sdk.Session",
      kind: RelationshipKind.REFERENCES,
      cardinality: Cardinality.MANY_TO_ONE,
      label: "SessionOptions.resume/continue/forkSession target a Session",
    },
    {
      id: "rel.session__message_types",
      from: "agent_sdk.Session",
      to: "agent_sdk.message_types",
      kind: RelationshipKind.COMPOSES,
      cardinality: Cardinality.ONE_TO_MANY,
      label: "Session .jsonl persists ordered SDKMessage history",
    },
    {
      id: "rel.query__hooks",
      from: "agent_sdk.query",
      to: "agent_sdk.HookSystem",
      kind: RelationshipKind.USES,
      cardinality: Cardinality.ONE_TO_ONE,
      label: "query() options.hooks registers HookCallbackMatcher[] per event",
    },
    {
      id: "rel.query__permissions",
      from: "agent_sdk.query",
      to: "agent_sdk.PermissionSystem",
      kind: RelationshipKind.USES,
      cardinality: Cardinality.ONE_TO_ONE,
      label: "query() options.permissionMode + canUseTool configure access control",
    },
    {
      id: "rel.hooks__session",
      from: "agent_sdk.HookSystem",
      to: "agent_sdk.Session",
      kind: RelationshipKind.REFERENCES,
      cardinality: Cardinality.MANY_TO_ONE,
      label: "Hook inputs include session_id, transcript_path from BaseHookInput",
    },
    {
      id: "rel.dim_sessions__session",
      from: "db.dim_sessions",
      to: "agent_sdk.Session",
      kind: RelationshipKind.REFERENCES,
      cardinality: Cardinality.MANY_TO_ONE,
